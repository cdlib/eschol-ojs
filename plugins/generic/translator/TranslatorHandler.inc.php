...
class TranslatorHandler extends Handler {
	...
	function index() {
		...
		$templateMgr->assign('locales', new ArrayItemIterator(\OjsLocale::getAllLocales(), $rangeInfo->getPage(), $rangeInfo->getCount()));
	...
	function setupTemplate($subclass = true) {
		...
		$templateMgr =& TemplateManager::getManager();
		\OjsLocale::requireComponents(array(LOCALE_COMPONENT_PKP_ADMIN, LOCALE_COMPONENT_PKP_MANAGER));
	...
	function edit($args) {
		...
		if (!\OjsLocale::isLocaleValid($locale)) Request::redirect(null, null, 'index');
