...
class StudentThesisForm extends Form {
	...
	function __construct($parentPluginName, $thesisId = null) {
		...
		$this->validDegrees = array (
			THESIS_DEGREE_MASTERS => \OjsLocale::translate('plugins.generic.thesis.manager.degree.masters'),
			THESIS_DEGREE_DOCTORATE => \OjsLocale::translate('plugins.generic.thesis.manager.degree.doctorate')
		);
		...
		$this->addCheck(new FormValidatorCustom($this, 'dateApprovedYear', 'required', 'plugins.generic.thesis.form.dateApprovedValid', function ($dateApprovedYear) {
      ...
      return $dateApprovedYear >= $minYear && $dateApprovedYear <= $maxYear ? true : false;
		...
		$this->addCheck(new FormValidatorCustom($this, 'dateApprovedMonth', 'required', 'plugins.generic.thesis.form.dateApprovedValid', function ($dateApprovedMonth) {
      return $dateApprovedMonth >= 1 && $dateApprovedMonth <= 12 ? true : false;
		...
		$this->addCheck(new FormValidatorCustom($this, 'dateApprovedDay', 'required', 'plugins.generic.thesis.form.dateApprovedValid', function ($dateApprovedDay) {
	...
	function readInputData() {
		...
		if (!empty($this->_data['url'])) {
			$this->addCheck(new FormValidatorCustom($this, 'url', 'required', 'plugins.generic.thesis.form.urlPrefixIncluded', function ($url) {
	...
	function execute() {
		...
		if (empty($uploadCode) || ($uploadCode != $submittedUploadCode)) {
			...
			if (!empty($thesisPhone)) {
				$thesisContactSignature .= "\n" . \OjsLocale::Translate('user.phone') . ': ' . $thesisPhone;
			}
			if (!empty($thesisFax)) {
				$thesisContactSignature .= "\n" . \OjsLocale::Translate('user.fax') . ': ' . $thesisFax;
			}

			$thesisContactSignature .= "\n" . \OjsLocale::Translate('user.email') . ': ' . $thesisEmail;
			...
			$paramArray = array(
				...
				'degree' => \OjsLocale::Translate($thesis->getDegreeString()),
