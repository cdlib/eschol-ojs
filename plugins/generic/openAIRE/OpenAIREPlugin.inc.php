...
class OpenAIREPlugin extends GenericPlugin {
	...
	function getDisplayName() {
		return \OjsLocale::translate('plugins.generic.openAIRE.displayName');
	}

	function getDescription() {
		return \OjsLocale::translate('plugins.generic.openAIRE.description');
	}	
	...
	function changeXML($hookName, $params) {
		...
		if ($openAIREDao->isOpenAIREArticle($article->getArticleId())) {
			...
			$rights = array_merge_recursive(
				empty($rights)?array(\OjsLocale::getLocale() => $openAIRERights):
