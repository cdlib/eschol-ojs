...
class PubMedExportPlugin extends ImportExportPlugin {
	...
	function getDisplayName() {
		return \OjsLocale::translate('plugins.importexport.pubmed.displayName');
	}

	function getDescription() {
		return \OjsLocale::translate('plugins.importexport.pubmed.description');
	}
...
//		$command = array_shift($args);
		...
		if (!$journal) {
			if ($journalPath != '') {
				echo \OjsLocale::translate('plugins.importexport.pubmed.cliError') . "\n";
				echo \OjsLocale::translate('plugins.importexport.pubmed.error.unknownJournal', array('journalPath' => $journalPath)) . "\n\n";
			}
		...
		if ($xmlFile != '') switch (array_shift($args)) {
			case 'articles':
				...
				if (!$this->exportArticles($results, $xmlFile)) {
					echo \OjsLocale::translate('plugins.importexport.pubmed.cliError') . "\n";
					echo \OjsLocale::translate('plugins.importexport.pubmed.export.error.couldNotWrite', array('fileName' => $xmlFile)) . "\n\n";
				}
				return;
			case 'issue':
				$issueId = array_shift($args);
				...
				if ($issue == null) {
					echo \OjsLocale::translate('plugins.importexport.pubmed.cliError') . "\n";
					echo \OjsLocale::translate('plugins.importexport.pubmed.export.error.issueNotFound', array('issueId' => $issueId)) . "\n\n";
					return;
				}
				$issues = array($issue);
	...
	function usage($scriptName) {
		echo \OjsLocale::translate('plugins.importexport.pubmed.cliUsage', array(
