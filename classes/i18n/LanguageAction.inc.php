...
class LanguageAction {
	...
	function getDownloadableLocales() {
		...
		return \OjsLocale::loadLocaleList($descriptorFilename);
	}
	...
	function downloadLocale($locale, &$errors) {
		...
		if (!is_array($downloadableLocales) || !isset($downloadableLocales[$locale])) {
			$errors[] = \OjsLocale::translate('admin.languages.download.cannotOpen');
			return false;
		}
		...
		$locales = \OjsLocale::getAllLocales();
		if (!isset($locales[$locale])) {
			...
			if ($pos === false) {
				...
				$errors[] = \OjsLocale::translate('admin.languages.download.cannotModifyRegistry');
				return false;
			}
