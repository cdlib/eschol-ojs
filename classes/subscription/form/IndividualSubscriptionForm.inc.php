...
class IndividualSubscriptionForm extends SubscriptionForm {
	...
	function __construct($subscriptionId = null, $userId = null) {
		...
		if ($subscriptionTypeCount == 0) {
			$this->addError('typeId', \OjsLocale::translate('manager.subscriptions.form.typeRequired'));
      ...
      return $subscriptionTypeDao->subscriptionTypeExistsByTypeId($typeId, $journalId) && $subscriptionTypeDao->getSubscriptionTypeInstitutional($typeId) == 0;
		...
		if (!isset($subscriptionId)) {
			...
			$this->addCheck(new FormValidatorCustom($this, 'userId', 'required', 'manager.subscriptions.form.subscriptionExists', function ($userId, $journalId, $subscriptionId) use ($subscriptionDao) {
       $subscriptionDao =& DAORegistry::getDAO('IndividualSubscriptionDAO');
