...
class SubscriptionType extends DataObject {
	...
	function getDurationYearsMonths() {
		if ($this->getData('nonExpiring')) {
			return \OjsLocale::translate('subscriptionTypes.nonExpiring');
		}
		...
		if ($years == 1) {
			$yearsMonths = '1 ' . \OjsLocale::translate('subscriptionTypes.year');
		} elseif ($years > 1) {
			$yearsMonths = $years . ' ' . \OjsLocale::translate('subscriptionTypes.years');
		}

		if ($months == 1) {
			$yearsMonths .= $yearsMonths == ''  ? '1 ' : ' 1 ';
			$yearsMonths .= \OjsLocale::translate('subscriptionTypes.month'); 
		} elseif ($months > 1){
			$yearsMonths .= $yearsMonths == ''  ? $months . ' ' : ' ' . $months . ' ';
			$yearsMonths .= \OjsLocale::translate('subscriptionTypes.months');
		}
