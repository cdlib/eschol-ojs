...
class AuthorSubmitStep1Form extends AuthorSubmitForm {
	...
	function AuthorSubmitStep1Form(&$article, &$journal) {
		parent::__construct($article, 1, $journal);
	...
	function display() {
		...
		$templateMgr->assign('sectionOptions', array('0' => \OjsLocale::translate('author.submit.selectSection')) + $sectionDao->getSectionTitles($journal->getId(), !$isEditor));
	...
	function initData() {
		if (isset($this->article)) {
			...
			$tryLocales = array(
				...
				\OjsLocale::getLocale(), // Current UI locale
