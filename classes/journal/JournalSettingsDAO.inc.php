...
class JournalSettingsDAO extends DAO {
		...
		foreach ($tree->getChildren() as $setting) {
			...
			if (isset($nameNode) && isset($valueNode)) {
				...
				$this->updateSetting(
					$journalId,
					$name,
					$isLocaleField?array(\OjsLocale::getLocale() => $value):$value,
					$type,
					$isLocaleField
				);
			}
	...
	function _performLocalizedReplacement($rawInput, $paramArray = array(), $locale = null) {
		...
		if ( isset($matches[1]) ) {
			\OjsLocale::requireComponents(array(LOCALE_COMPONENT_OJS_DEFAULT, LOCALE_COMPONENT_OJS_MANAGER), $locale);
			return \OjsLocale::translate($matches[1], $paramArray, $locale);
		}
	...
	function _installer_regexp_callback($matches) {
		return \OjsLocale::translate($matches[1]);
	}
}
