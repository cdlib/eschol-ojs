		...
		switch ($this->type) {
			...
			case PAYMENT_TYPE_SUBMISSION:
				if ( $journal->getLocalizedSetting('submissionFeeName') != '' ) {
					...
					return \OjsLocale::translate('payment.type.submission');
				}
	...
	function getAssocDescription() {
		...
		switch ($this->type) {
			...
			case PAYMENT_TYPE_RENEW_SUBSCRIPTION:
				...
				if ( !$subscription) return \OjsLocale::translate('manager.payment.notFound');
			...
			case PAYMENT_TYPE_PURCHASE_ARTICLE:
				...
				if ( !$article ) return \OjsLocale::translate('manager.payment.notFound');
