...
class IssueHandler extends Handler {
	...
	function current($args = null) {
		...
		if ($issue != null) {
			...
			$issueCrumbTitle = \OjsLocale::translate('current.noCurrentIssue');
			$issueHeadingTitle = \OjsLocale::translate('current.noCurrentIssue');
		}
	...
	static function setupIssueTemplate(&$issue, $showToc = false) {
		...
		if (isset($issue) && ($issue->getPublished() || Validation::isEditor($journalId) || Validation::isLayoutEditor($journalId) || Validation::isProofreader($journalId)) && $issue->getJournalId() == $journalId) {
			...
			$issueCrumbTitle = \OjsLocale::translate('archive.issueUnavailable');
			$issueHeadingTitle = \OjsLocale::translate('archive.issueUnavailable');
		}
	...
	function archive() {
		...
		$templateMgr =& TemplateManager::getManager();
		$templateMgr->assign('coverPagePath', $coverPagePath);
		$templateMgr->assign('locale', \OjsLocale::getLocale());
	...
	function setupTemplate() {
		...
		\OjsLocale::requireComponents(array(LOCALE_COMPONENT_PKP_READER, LOCALE_COMPONENT_OJS_EDITOR));
	}
}
