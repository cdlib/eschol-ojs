...
class AdminLanguagesHandler extends AdminHandler {
	...
	function languages($args, &$request) {
		...
		foreach (\OjsLocale::getAllLocales() as $key => $name) {
			$localesComplete[$key] = \OjsLocale::isLocaleComplete($key);
		}
		$templateMgr->assign('localesComplete', $localesComplete);

		$templateMgr->assign('installedLocales', $site->getInstalledLocales());
		$templateMgr->assign('uninstalledLocales', array_diff(array_keys(\OjsLocale::getAllLocales()), $site->getInstalledLocales()));
	...
	function saveLanguageSettings($args, &$request) {
		...
		if (\OjsLocale::isLocaleValid($primaryLocale)) {
		...
		if (isset($supportedLocales) && is_array($supportedLocales)) {
			foreach ($supportedLocales as $locale) {
				if (\OjsLocale::isLocaleValid($locale)) {
	...
	function installLocale($args, &$request) {
		...
		if (isset($installLocale) && is_array($installLocale)) {
			...
			foreach ($installLocale as $locale) {
				if (\OjsLocale::isLocaleValid($locale) && !in_array($locale, $installedLocales)) {
	...
	function downloadLocale($args, &$request) {
		...
		import('lib.pkp.classes.notification.NotificationManager');
		$notificationManager = new NotificationManager();
		$notificationManager->createTrivialNotification(\OjsLocale::translate('notification.notification'), \OjsLocale::translate('admin.languages.localeInstalled', array('locale' => $locale)), NOTIFICATION_TYPE_SUCCESS, null, false);
		$request->redirect(null, null, 'languages');
	}
}
